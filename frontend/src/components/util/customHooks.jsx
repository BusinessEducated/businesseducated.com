...
export const useFormStoreInput = (formName, name, placeholder) => {
  ...
  const onChange = (e) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.info(`form: ${formName}, ${name}: ${value}`)
    }
  }
...
export const toggleTextHighlight = (defaultValue = false) => {
  ...
  useEffect(() => {
    if (highlightEnabled)
      setHighlightEnabled({ enabled: !highlightEnabled.enabled, styles: {} })
  ...
  handler = (entries, observer = null) =>
    entries.map((entry) => {
      ...
      if (entry.intersectionRatio >= 0.5)
        console.info('visible: ', entry.target)
      return ''
    }),
  options = {
  ...
  switch (validationType) {
    case 'string':
      return Yup.string()
        ...
        .matches(validationRegex, validationMessage)
    case 'number':
      return Yup.number()
        .required(validationRequired)
        .matches(validationRegex, validationMessage)
    case 'boolean':
      return Yup.boolean().required(validationRequired)
    default:
      return Yup.mixed().required(validationRequired)
  }
}
...
export const useFormStore = (
  ...
  const [input, setFormInput] = useState(
    stateDefaultValue || useStore((state) => state[formName][fieldName]),
  )
  const changeFormData = useStore(
    (state) => state[formName].methods.changeFormData,
  )
  const [error, setError] = useState(false)

  useEffect(() => {
    if (process.env.NODE_ENV === 'development')
      console.log(`New Form Input: ${input} for form ${formName}`)

    // test input for errors

    yup
      .string(validationTypes[validationType])
      .validate(input, { abortEarly: false, strict: true })
...
export const useFormStoreNoValidation = (
  formName = 'testForm',
  fieldName = 'text',
  stateDefaultValue = false,
) => {
