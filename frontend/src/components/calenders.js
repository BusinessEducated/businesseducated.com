...
//#region calender week
function classNames2(...classes) {
  return classes.filter(Boolean).join(' ')
}

export const CalenderWeek = () => {
            ...
            {dayss.map((day, dayIdx) => (
              <button
                ...
                className={classNames3(
                  ...
                  dayIdx === dayss.length - 1 && 'rounded-br-lg'
                )}
              >
                <time
                  dateTime={day.date}
  ...
  return (
    <div>
      <div className="relative grid grid-cols-1 gap-x-14 md:grid-cols-2">
        ...
        {months2.map((month, monthIdx) => (
          <section
            key={monthIdx}
            className={classNames5(monthIdx === months.length - 1 && 'hidden md:block', 'text-center')}
          >
            <h2 className="font-semibold text-gray-900">{month.name}</h2>
            <div className="mt-6 grid grid-cols-7 text-xs leading-6 text-gray-500">
              <div>M</div>
