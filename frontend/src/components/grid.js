            ...
            <div className="flex flex-1 items-center justify-between truncate rounded-r-md border-t border-r border-b border-gray-200 bg-white">
              ...
              <div className="flex-shrink-0 pr-2">
                ...
                >
                  ...
                  <div
                    dangerouslySetInnerHTML={{ __html: ellipsesVerticalIcon }}
                    className="h-5 w-5"
                    aria-hidden="true"
                  />
                </button>
...
export const GridList = ({
  ...
  return (
    ...
    >
      {people.map(
        ({ email, imageUrl, name, roles, telephone, description, title }) => (
          <li
            key={email}
            className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow"
          >
            <div className="flex flex-col w-full items-center justify-between space-x-6 pb-6 hover:-translate-y-5 translate-y-0 ease-in-out transition-all">
              <img
                className="h-64 w-full object-contain rounded-t-md bg-red-300"
                src={imageUrl}
                alt=""
              />

              <div className="w-full pt-5 flex flex-col h-full overflow-hidden mb-12">
                <p className="mt-1 text-sm text-gray-500 w-full">{title}</p>
                <h3 className="text-lg font-bold text-gray-900 mb-2">{name}</h3>

                <div className="flex items-center space-x-3 overflow-x-scroll scroll-smooth -scroll-mb-16 scrollbar-hide">
                  {roles.map((role) => (
                ...
                <div className="flex divide-x divide-gray-200">
                  {email && (
                    <div className="flex w-0 flex-1">
                      <a
                        href={`mailto:${email}`}
                        className="relative inline-flex flex-1 items-center justify-center rounded-bl-lg border border-transparent py-4 text-sm font-medium text-gray-700 hover:text-gray-500"
                      >
                        <EnvelopeIcon
                          className="h-5 w-5 text-gray-400"
                          aria-hidden="true"
